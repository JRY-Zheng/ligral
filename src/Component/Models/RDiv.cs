...
namespace Ligral.Component.Models
{
    class RDiv : Model
    {
        protected override string DocString
        {
            get
            {
                return "This model outputs the matrix product of the inverse of the first input and the second input";
            }
        }
        ...
        public override void Check()
        {
            ...
            else if (InPortList[0].ColNo == InPortList[0].RowNo && InPortList[0].ColNo == InPortList[1].RowNo)
            {
        ...
        protected override List<Matrix<double>> Calculate(List<Matrix<double>> values)
        {
            try
            {
                Results[0] = values[0].LeftDiv(values[1]);
            }
